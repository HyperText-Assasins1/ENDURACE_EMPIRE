...
def add_ExerciseSet_action(id):
    ...
    create_exerciseSet(setName, user.id, id)
...
def exercise_info(exercise):
    ...
    if user:
        ...
        flash('Invalid or unauthorize')
        return rediect(url_for('index_views.index_page'))

@index_views.route('/rename-exerciseSet/<exerciseSet_id>', methods = ['POST'])
@login_required
def rename_exerciseSet_Action(exerciseSet_id):

    data = request.form

    oldName = get_exerciseSet_by_id(exerciseSet_id)

    res = rename_exerciseSet(oldName.name, data['rename'])

    if(res):
        flash('ExerciseSet Renamed!')
    else:
        flash('ExerciseSet not renamed or Unauthorized')
    return redirect(url_for('index_views.index_page'))
